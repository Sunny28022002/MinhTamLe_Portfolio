...
import { Toast, Spin, Select } from "@douyinfe/semi-ui";
...
const UpdateRolePage = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${userServiceAPI}/Role/id?id=${roleId}`,
          {
        ...
        formik.setFieldValue("user", response.data.data.user);
        formik.setFieldValue("blog", response.data.data.blog);
        formik.setFieldValue("medicine", response.data.data.medicine);
        formik.setFieldValue(
          "examinatedRecord",
          response.data.data.examinatedRecord
        );
        formik.setFieldValue("feedback", response.data.data.feedback);
        formik.setFieldValue("schedule", response.data.data.schedule);
        formik.setFieldValue("chat", response.data.data.chat);
        formik.setFieldValue(
          "isActive",
          response.data.data.isActive ? "1" : "0"
        );
        setSpinner(false);
      } catch (error) {
        setSpinner(false);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      setSpinner(true);
      axios
        .put(`${userServiceAPI}/Role/Update`, values, {
          headers: headerConfig,
        })
        .then((response) => {
    ...
    <>
      {spinner ? (
        ...
        <div className="flex flex-col my-7 items-center w-full">
          <HeaderAdminManagementComponent content={"Update Role"} />
          <form
...
export default withAuth(UpdateRolePage, adminRole);
