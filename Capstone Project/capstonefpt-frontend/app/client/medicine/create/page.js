...
const CreateMedicineClientPage = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      setSpinner(true);
      axios
        .post(`${medicineServiceAPI}/Medicine/Create`, values, {
          headers: headerConfig,
        })
        .then((response) => {
    ...
    <>
      {spinner ? (
        ...
        <div className="flex flex-col my-7 items-center w-full">
          {/* -------- Title -------- */}
          <HeaderAdminManagementComponent content={"Create Medicine"} />
          {/* Form */}
          <form
...
export default CreateMedicineClientPage;
